"""Logico CLI — read, compare, sync, and version-control Logic Pro / Dorico / StaffPad projects."""
...
def cmd_diff(args: list[str]) -> None:
    ...
    if len(args) < 2:
        ...
        print("       logico diff <file> @N           (current vs snapshot N)")
        print("       logico diff <file> @N @M        (snapshot N vs snapshot M)")
        sys.exit(1)
...
def cmd_log(args: list[str]) -> None:
    ...
    if not numbers:
        print(f"No snapshots for {file_path}. Run 'logico sync' to create one.")
        return
...
def cmd_revert(args: list[str]) -> None:
    ...
    if len(args) < 2:
        print("Usage: logico revert <file> @N")
        sys.exit(1)
...
def cmd_watch(args: list[str]) -> None:
    ...
    if len(args) < 2:
        print("Usage: logico watch <source> <destination>")
        sys.exit(1)
...
def main() -> None:
    ...
    if not args or args[0] in ("-h", "--help", "help"):
        ...
        print("  logico diff <file> @N           Compare current state vs snapshot N")
        print("  logico diff <file> @N @M        Compare two snapshots")
        print("  logico sync <source> <dest>     Sync notes from source to destination")
        print("  logico log <file>               List all snapshots for a project")
        print("  logico revert <file> @N         Restore a project to snapshot N")
        print("  logico watch <source> <dest>    Auto-sync on every save")
        print()
