...
Config file (logico.toml, searched from the current directory upward):
...
When syncing, Logico uses this table to match source tracks to destination
...
def _find_config(start: Path | None = None) -> Path | None:
    """Search current dir and parents for logico.toml."""
...
def load_mapping(config_path: Path | None = None) -> list[dict[str, str]]:
    """Load track mappings from logico.toml.
...
def resolve_track_name(
    ...
    Resolution order:
    1. Explicit mapping in logico.toml
