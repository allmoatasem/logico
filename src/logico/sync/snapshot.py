"""Snapshot system for Logico version history.

Each project file gets a `.logico/<filename>/` directory next to it.
...
def _logico_dir(file_path: str | Path) -> Path:
    """Return the .logico/<filename>/ directory for a project file."""
    p = Path(file_path).resolve()
    return p.parent / ".logico" / p.name


def _snapshot_path(file_path: str | Path, number: int) -> Path:
    return _logico_dir(file_path) / f"{number:03d}.json"


def list_snapshots(file_path: str | Path) -> list[int]:
    """Return a sorted list of snapshot numbers for a project file."""
    d = _logico_dir(file_path)
    if not d.exists():
        return []
    numbers = []
    for f in d.iterdir():
